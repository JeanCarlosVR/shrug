...
class Cluster {
    ...
    public connect(firstShardID, lastShardID, maxShards, token, type, clientOptions) {
        ...
        bot.on("shardDisconnect", (err, id) => {
            ...
            let embed = {
                description: `☠️ Shard ${id} is dead.`,
                color: 0xFF0000
            }
        ...
        bot.on("shardReady", id => {
            ...
            let embed = {
                description: `💖 Shard ${id} is now up.`,
                color: 0x32FF00
            }
            process.send({ name: "shard", embed: embed });
        });
        ...
        bot.on("ready", id => {
            ...
            /*let embed = {
                description: `**${this.lastShardID + 1}** Shards initializated in the cluster **${this.clusterID}**.`,
                color: 0x32FF00
            }
    ...
    public startStats(bot) {
        setInterval(() => {
            ...
            this.shardsStats = [];
            for(let shard of this.bot.shards) {
